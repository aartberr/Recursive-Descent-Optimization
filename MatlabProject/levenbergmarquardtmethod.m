function xk=levenbergmarquardtmethod(f,xs,e,gama,abc)
  ...
  while e<=norm(gradfxk) && k<=maxk%Αν ε<=|gradient(f(xk))| συνεχίζει το loop
    ...
    eighess=eig(hessfxk);
    maxeig=abs(eighess(1,1));
    for i=2:size(eighess,2) %βρίσκουμε την μεγαλύτερη ιδιοτιμή για το κάτω όριο του μκ
      if abs(eighess(i,1))>maxeig
         maxeig=abs(eighess(i,1));
      end
    end
    mk=maxeig+1;
    dk=-(hessfxk+mk*eye)\gradfxk; %δκ=-inverse(hessian(f(xk))+μκ*I)gradient(f(xk)
    d=double(dk);
    if abc==1
          gk(k)=gama;
      elseif abc==2
